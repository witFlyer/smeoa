<?php
class OrgTaxAction extends CommonAction {
...
//		$this->_edit();
		...
		if ($id > 0)
		{
			$detail = D("OrgTax")->find(array("where" => "id='$id'"));
	...
	{
		$orgId = isset($_REQUEST['org_id']) ? intval($_REQUEST['org_id']) : 0;
		$list  = D("OrgTax")->findAll(array("where" => "org_id='$orgId'"));
	...
	{
		$id     = isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
		$result = D("OrgTax")->delete(array('where' => "id=$id"));
