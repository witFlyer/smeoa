...
class ServiceTypeAction extends CommonAction {
	...
	function edit() {
		...
		foreach ($person_list as $k=>$v){
			$list=M('UserService')->where(array('flow_type_id'=>$id,'user_id'=>$v['id']))->find();
			if($list){
				$person[$k]=$v;
		...
		$this->_assign_role_list();
		$this -> display();
	}
	protected function _assign_role_list() {
		$model = D("Role");
		$where['is_del']=array('eq',0);
		$role_list = $model ->where($where)->order('sort')->getField("id,name");
...
//删除草稿数据
	public function del_draft(){
		if($this->isAjax()){
			$id=I('post.id','','int');
			$where=array('id'=>$id);
			$k = M('Service')->where($where)->delete();
			if($k){
				echo 1;
			}else{
				echo 0;
