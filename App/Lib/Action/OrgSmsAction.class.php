<?php
class OrgSmsAction extends CommonAction {
		...
		if ($result['error'] == 0)
		{
			...
			if ($data)
			{
				...
				foreach ($data AS $line)
				{
					...
					if (count($arr) == 2 AND is_mobile($arr[1]))
					{
						$html[] = $arr;
					}
				}
				...
				if ($html)
				{
					...
					setcookie('org_sms_import', $html, NULL, '/');
				}
			}
...
//		$this->assign('jumpUrl', get_return_url());
		$this->assign('jumpUrl', U('org_sms/index'));
		$this->success('新增成功!');
	}
...
//		$map['mobile'] = array(array('NEQ', ''), array('EXP', NULL));
	...
	function _search_filter(&$map)
	{
		...
		if (isset($_REQUEST['mobile']) AND ! empty($_REQUEST['mobile']) AND empty($map['mobile']))
		{
			$map['mobile'] = array('like', "%" . $_POST['mobile'] . "%");
		}
		...
		if (isset($_REQUEST['send_message']) AND ! empty($_REQUEST['send_message']) AND empty($map['send_message']))
		{
			$map['send_message'] = array('like', "%" . $_POST['send_message'] . "%");
		}
