...
class PopupAction extends CommonAction {
	...
	function saveType(){
		if($this->isAjax()){
			...
			if($_POST['actionName']=="ServiceType"){
				$k = M('UserService')->where(array('user_id'=>$data['user_id'],'flow_type_id'=>$data['flow_type_id']))->find();
				if(!empty($k)){
					$list=M('UserService')->where(array('user_id'=>$data['user_id'],'flow_type_id'=>$data['flow_type_id']))->save($data);	
				}else{
					$list=M('UserService')->add($data);
				}
				if(false !==$list){
					echo '1';
				}else{
				...
				$k = M('UserFlow')->where(array('user_id'=>$data['user_id'],'flow_type_id'=>$data['flow_type_id']))->find();
				if(!empty($k)){
					$list=M('UserFlow')->where(array('user_id'=>$data['user_id'],'flow_type_id'=>$data['flow_type_id']))->save($data);	
				}else{
					$list=M('UserFlow')->add($data);
				}
				if(false !==$list){
					echo '1';
				}else{
