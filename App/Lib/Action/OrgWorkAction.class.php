<?php
class OrgWorkAction extends CommonAction {
...
//		$this->_edit();
		...
		if ($id > 0)
		{
			$detail = D("OrgWork")->find(array("where" => "id='$id'"));
	...
	{
		$orgId = isset($_REQUEST['org_id']) ? intval($_REQUEST['org_id']) : 0;
		$list  = D("OrgWork")->findAll(array("where" => "org_id='$orgId'"));
	...
	{
		$id     = isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
		$result = D("OrgWork")->delete(array('where' => "id=$id"));
