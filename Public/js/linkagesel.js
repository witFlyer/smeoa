...
//	this.data		= {'0': {'name': 'root', val: 0, cell: {}} };		// 数据根 ajax get0时需要后台处理为获取DB第一级
	...
	if(opts && typeof opts === 'object') {  
		$.extend(true, this.st, opts);
	}
	
	var count = 0;
	if (this.st.dataReader) {
		for (k in this.st.dataReader) {
			if (this.st.dataReader.hasOwnProperty(k)) {
				count++;
			}
		}
	}
...
//			});
		...
		if (st.autoLink && index === 2 && st.autoOne) {		// 只有一个选项的直接选中 并且联动下级
			selectedIdx--;	// !
			bindEl.value = x;
...
LinkageSel.prototype.clean = function(bindIdx) {
	...
	for (var i = len - 1; i > bindIdx; i--) {
		...
		if (elm[0] && elm.length) {	// ?length
			...
			topt = elm.children();
			// uniform
			if (topt.parent().prev("span").parent("div").hasClass("selector")) {
				topt.parent().prev("span").html("");
			}
			topt.remove();
